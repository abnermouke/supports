...
class RepositoryModule
{
    ...
    public function __construct($model)
    {
        ...
        $this->__init($model);
        //设置默认表链接别名
        $this->connetion = $model::DB_CONNECTION;
        //设置表名
        $this->table_name = $model::TABLE_NAME;
        //设置信息
        $this->default_model = $this->model = $model;
        //设置模型
        $this->model();
    }
     ...
     * @Time 2023-04-20 14:26:46
     * @return bool
     */
    private function __init()
    {
        //设置实例
        $orm_capsule = new Manager();
        //加载配置
        $this->loadConnection($orm_capsule);
        //设置监听
        $orm_capsule->setEventDispatcher((new Dispatcher(new Container())));
        //设置全局可静态访问
        $orm_capsule->setAsGlobal();
        //启用模型
        $orm_capsule->bootEloquent();
        //返回成功
        return true;
    }
     ...
     * @Author Abnermouke <abnermouke@outlook.com | yunnitec@outlook.com>
     * @Originate in YunniTec <https://www.yunnitec.com/>
     * @Time 2023-04-20 14:26:14
     * @param $orm_capsule Manager Manage（DB）实例
     * @return mixed
     */
    private function loadConnection($orm_capsule)
    {
        //引入配置
        foreach (include Path::root('config').DIRECTORY_SEPARATOR.'orm_database.php' as $name => $confg) {
            //设置配置
            $confg && $orm_capsule->addConnection($confg, $name);
        }
        //返回成功
        return $orm_capsule;
    }
    ...
    {
        //设置当前模型
        return ($this->model = $this->default_model);
    }
    ...
    {
        // 执行自定义语句
        return Manager::connection($connection)->statement($sql);
    }
    ...
    private function setOrders($query, $orders = [])
    {
        ...
        foreach ($orders as $field => $order) {
            //根据类型处理
            switch ($order) {
                case ModelModule::LATEST_ORDER_BY:
                    //执行倒序排序
                    $query = $query->latest();
                    break;
                case ModelModule::OLDEST_ORDER_BY:
                    //执行正序排序
                    $query = $query->oldest();
                    break;
                case ModelModule::RANDOM_ORDER_BY:
                    //执行随机排序
                    $query = $query->inRandomOrder();
                    break;
                case ModelModule::RAW_ORDER_BY:
                    //执行自定义排序
